...
describe( 'accessor multiply', function tests() {
	...
	it( 'should throw an error if provided a typed array to be multiplied which is not of equal length to the input array', function test() {
		expect( foo ).to.throw( Error );
		function foo() {
			multiply( [], [1,2], new Int32Array( [1,2,3] ), getValue );
		}
		function getValue( d ) {
			return d;
		}
