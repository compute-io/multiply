...
var // Expectation library:
	...
	// Module to be tested:
	multiply = require( './../lib/matrix.js' );
...
describe( 'matrix multiply', function tests() {
	...
	it( 'should throw an error if provided a matrix to be multiplied which is not of equal length to the input matrix', function test() {
		expect( badValues ).to.throw( Error );
		function badValues() {
			multiply( matrix( [5,5] ), mat, matrix( [10,10] ) );
		}
	});

	it( 'should multiply each matrix element with a scalar', function test() {
		var actual;

		actual = matrix( [5,5], 'float64' );
		actual = multiply( actual, mat, 1 );

		assert.deepEqual( actual.data, out2.data );
	});

	it( 'should multiply two matrices by each other element-wise', function test() {
		var actual;

		actual = matrix( [5,5], 'float64' );
		actual = multiply( actual, mat, mat );
	...
	it( 'should return an empty matrix if provided an empty matrix', function test() {
		...
		mat = matrix( [0,10] );
		assert.deepEqual( multiply( out, mat, 1 ).data, expected );
